// memmgr.cc: Implement the cross-platform memory management routines.
// The hashing routines are taken from xxHash (https://github.com/Cyan4973/xxHash).
#include <string.h>
...
{
    return (block->BytesCommitted > 0 || block->BytesReserved > 0) ? 1 : 0;
}
...
{
    return (old_block->HostAddress != new_block->HostAddress) ? 1 : 0;
}
...
)
{
    ...
    if (init == nullptr || o_arena == nullptr) {
        assert(init != nullptr);
        assert(o_arena != nullptr);
        return -1;
    }
    if (init->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_INVALID) {
        assert(init->AllocatorType != PIL_MEMORY_ALLOCATOR_TYPE_INVALID);
        return -1;
    }
    if (init->ArenaFlags == PIL_MEMORY_ARENA_FLAGS_NONE) {
        assert(init->ArenaFlags != PIL_MEMORY_ARENA_FLAGS_NONE);
        return -1;
    }
    if (init->ArenaFlags & PIL_MEMORY_ARENA_FLAG_EXTERNAL) {
        if (init->ArenaFlags & PIL_MEMORY_ARENA_FLAG_INTERNAL) {
            assert(0 && "ArenaFlags cannot specify both INTERNAL and EXTERNAL");
            return -1;
        }
        if (init->AllocatorType != PIL_MEMORY_ALLOCATOR_TYPE_DEVICE) {
            if (init->MemoryStart.HostAddress == nullptr) {
                assert(init->MemoryStart.HostAddress != nullptr);
                return -1;
            }
        }
    }
    if (init->ArenaFlags & PIL_MEMORY_ARENA_FLAG_INTERNAL) {
        if (init->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_DEVICE) {
            assert(0 && "PIL_MEMORY_ALLOCATOR_TYPE_DEVICE cannot specify PIL_MEMORY_ARENA_FLAG_INTERNAL");
            return -1;
        }
    }
    ...
    if (init->ArenaFlags & PIL_MEMORY_ARENA_FLAG_INTERNAL) {
        ...
        if (init->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_HEAP) {
            if ((host_addr = PIL_HostMemoryAllocateHeap(&block, nb_commit, 16)) == nullptr) {
                return -1;
            }  alloc_flags = PIL_HOST_MEMORY_ALLOCATION_FLAGS_READWRITE;
        } else if (init->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_VMM) {
            if ((host_addr = PIL_HostMemoryReserveAndCommit(&block, nb_reserve, nb_commit, init->AllocationFlags)) == nullptr) {
                return -1;
            }  alloc_flags = init->AllocationFlags;
        } else {
            assert(init->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_HEAP || init->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_VMM);
            return -1;
        }
    ...
    if (new_offset >= arena->MaximumOffset) {
        ...
        if (arena->NbCommitted  != arena->NbReserved) {
            ...
            if (min_amount <= max_amount) {
                ...
                if (PIL_HostMemoryIncreaseCommitment(&block, &block, new_amount)) {
...
)
{
    ...
    if (PIL_MemoryArenaAllocate(o_block, arena, size, alignment) == 0) {
...
)
{
    ...
    assert(marker.Arena->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_HEAP || marker.Arena->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_VMM);
...
{
    assert(marker1.Arena != nullptr);
    assert(marker2.Arena != nullptr);
    assert(marker1.Arena == marker2.Arena);
    assert(marker1.Arena->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_HEAP || marker1.Arena->AllocatorType == PIL_MEMORY_ALLOCATOR_TYPE_HOST_VMM);
